...
func _test_asset_catalog_manifest() -> void:
	...
	_assert(catalog.asset_exists("characters", "pond_friend"), "asset manifest should point to placeholder rest-day pond friend")
	...
	_assert(sprite_layer.has_sprite("pond_friend"), "VillageSpriteLayer should include the rest-day pond friend")
	...
	var pond_friend = sprite_layer.sprites.get("pond_friend")
	_assert(is_instance_valid(pond_friend) and pond_friend.has_meta("visible_when"), "pond friend should have manifest-driven rest-day visibility")
	var rest_state = VillageState.new()
	sprite_layer.apply_village_state(rest_state)
	_assert(is_instance_valid(pond_friend) and pond_friend.visible, "pond friend should be visible for the default rest message")
	rest_state.add_resident_message("花がひとつ咲きました。小さな一歩のしるしです。")
	sprite_layer.apply_village_state(rest_state)
	_assert(is_instance_valid(pond_friend) and not pond_friend.visible, "pond friend should step back after a growth resident message")
