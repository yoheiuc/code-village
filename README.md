...
村には、灯り、図書館、水辺、道端に寄ってくる小さな生き物も暮らします。彼らは作業量を評価せず、村にいる気配と休息のために存在します。
...
- 小さな生き物や resident の気配で、責めない癒しを出す
...
- 生き物の方向性: [docs/creature_companions.md](docs/creature_companions.md)
- 将来の常駐モード: [docs/ambient_desktop_mode.md](docs/ambient_desktop_mode.md)
- 将来のBGM/SE: [docs/audio_spec.md](docs/audio_spec.md)
...
一括のデバッグ確認は次の command を使います。実ユーザー save / inbox を触らず、temporary path で hook/import/save まで確認します。
...
9. Later: 小型常駐ウィンドウ、native menu bar / Dock 検証、BGM/SE の healing audio 化
...
無料で使われる水準、有料で渡せる水準、参照カテゴリは [docs/product_milestones.md](docs/product_milestones.md) に整理しています。
サブエージェントの 6 役割と handoff は [docs/agent_operating_model.md](docs/agent_operating_model.md) を参照してください。
