...
商品判断の gate は `docs/product_milestones.md`、サブエージェントの役割分担は `docs/agent_operating_model.md` を参照する。
...
7. Ambient desktop mode
   - まず Godot-only の小型常駐ウィンドウを検証する。
   - true menu bar extra / Dock integration は Swift / AppKit helper や native plugin の feasibility を見て判断する。
   - Launch at login、常時表示、background audio は明示 opt-in にする。
8. Healing audio
   - BGM / SE / ambient sound は local bundled asset のみで扱う。
   - mute、volume、play / pause、SFX rate limit、background audio opt-in を必須にする。
   - 音源の license / originality review を production asset gate に含める。
...
13. 外部 streaming 音源、system audio capture、microphone input
14. 明示 opt-in なしの launch at login、常時表示、background audio
15. 既存ゲームや公式アプリを想起させる BGM / SE / sound design のコピー
...
- `area:audio`: BGM、SE、ambient sound、audio settings。
