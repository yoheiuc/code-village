...
- Ambient audio / focus companion: 常時開いても邪魔にならない音量、低刺激の loop、明確な mute / pause。
...
- 常駐モードと BGM/SE は未実装でもよいが、Later 方針、opt-in、privacy boundary、copy 禁止が docs にある。
...
- 小型常駐表示または同等の desktop companion 体験があり、event reaction は 1-3 秒程度で非侵入的。
- BGM / SE / ambient sound に mute、volume、play / pause、background audio opt-in があり、音源の license / originality review が完了している。
- 常駐時の CPU / battery / save write / audio 多重再生が QA gate を通る。
...
- Later: ambient desktop mode、healing audio、native macOS menu bar / Dock integration は Now gate 後に進める。
