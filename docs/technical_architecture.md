...
保存先は `user://code_village_save.json`。JSON は `schema_version`, `settings`, `repositories`, `onboarding_guide_dismissed`, `imported_activity_event_ids`, `claude_activity_import_checkpoint`, `village_state`, `activity_events`, `growth_events` を持つ。`settings.auto_import_claude_events` は true 既定で、ローカル inbox の起動時/定期取り込みだけを制御する。Settings の toggle から変更し、手動 import はこの設定が off でも使える。
...
`claude_activity_import_checkpoint` は長時間起動向けの tail read checkpoint。raw inbox path は保存せず、`path_hash`, `offset`, `file_size`, `modified_time`, `updated_at` だけを保存する。
...
長時間起動と大きな inbox に備え、ingestor は `claude_activity_import_checkpoint` の byte offset から tail read する。checkpoint は append-only inbox を前提にし、file size が offset より小さい場合や path hash が変わった場合は先頭から読み直す。oversized / malformed line は無視し、checkpoint を進めることで同じ壊れた行を毎回読み直さない。`imported_activity_event_ids` は近い履歴の重複抑制、checkpoint は古い行の再取り込み防止として併用する。
